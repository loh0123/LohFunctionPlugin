...
enum class ELFPStreamDisconnectFlags : uint8
{
	...
	, LFP_SubsystemInvalid UMETA ( DisplayName = "Subsystem Invalid" )
	, LFP_FailToCreate UMETA ( DisplayName = "Fail To Create" )
};
...
enum class ELFPStreamSocketState : uint8
{
	...
	, LFP_Closed UMETA ( DisplayName = "Closed" )
};
...
struct FLFPStreamSocketPtrHandle
{
	...
	FLFPStreamSocketPtrHandle ( ) = default;
...
private:
	...
	UPROPERTY ( )
	ELFPStreamDisconnectFlags DisconnectFlags = ELFPStreamDisconnectFlags::LFP_None;
...
public:
	...
	UPROPERTY ( )
	int32 LastBytesSendOrReceive = 0;
...
public:
	...
	FORCEINLINE ELFPStreamDisconnectFlags GetDisconnectedReason ( ) const
	{
	...
	FORCEINLINE bool HasClosed ( ) const
	{
		return State == ELFPStreamSocketState::LFP_Closed;
	}
	...
	FORCEINLINE void MarkIdle ( )
	{
		State = ELFPStreamSocketState::LFP_Idle;
	}

	FORCEINLINE void MarkIncomingPackage ( )
	{
		State = ELFPStreamSocketState::LFP_IncomingPkg;
	}

	FORCEINLINE void MarkClosing ( const ELFPStreamDisconnectFlags Reason )
	{
		DisconnectFlags = Reason;

		State = ELFPStreamSocketState::LFP_Closing;
	}

	FORCEINLINE void MarkClosed ( )
	{
		State = ELFPStreamSocketState::LFP_Closed;
	}
...
public:
	...
	FORCEINLINE void CloseUpSocket ( ISocketSubsystem* SocketSubsystem , const bool bForce = false );

	FORCEINLINE void CleanUpClientSocket ( );

public:

	FORCEINLINE bool HasClosed ( ) const
	{
		return MainSocket.HasClosed ( );
	}
