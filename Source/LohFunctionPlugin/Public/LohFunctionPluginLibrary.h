...
public:
	...
	FORCEINLINE bool operator==( const FLFPCompactMetaData& Other ) const {
		return MetaTag == Other.MetaTag && MetaData == Other.MetaData && MetaType == Other.MetaType;
	}
};

USTRUCT( BlueprintType )
struct LOHFUNCTIONPLUGIN_API FLFPCompactMetaIDArray : public FLFPCompactIDArray
{
	GENERATED_USTRUCT_BODY()
...
public:
	...
	FORCEINLINE void RemoveItem( const int32 CompactIndex )
	{
		if ( IsValidIndex( CompactIndex ) == false )
		{
			UE_LOG( LFPCompactMetaArray , Warning , TEXT( "FLFPCompactMetaIDArray : RemoveItem Has Invalid CompactIndex (%d)" ) , CompactIndex );
	...
	FORCEINLINE void AddItem( const int32 CompactIndex , const FLFPCompactMetaData& NewItem )
	{
		if ( IsValidIndex( CompactIndex ) == false )
		{
			UE_LOG( LFPCompactMetaArray , Warning , TEXT( "FLFPCompactMetaIDArray : AddItem Has Invalid CompactIndex (%d)" ) , CompactIndex );
