...
public:
	...
	/** Whether this buffer is currently visible */
	bool bBufferVisible;
};
...
{
public:
	FLFPBaseVoxelMeshSceneProxy(ULFPBaseVoxelMeshComponent* Component, const int32 BufferCount) : FPrimitiveSceneProxy(Component), VoxelComponent(Component), MaterialRelevance(Component->GetMaterialRelevance(GetScene().GetFeatureLevel()))
	{
		//AllocatedBufferSets.Init(nullptr, BufferCount * Component->GetNumMaterials());

		AllocatedBufferSets = { new FLFPVoxelMeshRenderBufferSet(GetScene().GetFeatureLevel()) };

		FLFPVoxelMeshRenderBufferSet* Buffer = AllocatedBufferSets[0];
		...
		//Buffer->StaticMeshVertexBuffer.SetUseFullPrecisionUVs(true);
		...
		FMeshElementCollector& Collector) const override
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			if (VisibilityMap & (1 << ViewIndex))
			{
				...
				{
					if (BufferSet == nullptr || BufferSet->bBufferVisible == false)
					{
						continue;
					}
