...
struct LOHFUNCTIONPLUGIN_API FLFPMazeStartData
{
	...
	FLFPMazeStartData( const FIntVector Loc , const TArray< int32 >& Open ) :
		GraphLocation(Loc), OpenConnection(Open)
	{
	}

	FLFPMazeStartData( const FIntVector Loc , const TArray< int32 >& Open , EMazeCellType Type ) :
		GraphLocation(Loc), OpenConnection(Open), CellType(Type)
	{
	}
