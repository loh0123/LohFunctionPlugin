	...
	for ( int32 Z = 0 ; Z < MazeSize.Z ; Z++ )
	{
		for ( int32 Y = 0 ; Y < MazeSize.Y ; Y++ )
		{
			for ( int32 X = 0 ; X < MazeSize.X ; X++ )
			{
				...
				if ( X - 1 >= 0 )
				{
					Open.Emplace ( Index - 1 );
				}
				if ( Y - 1 >= 0 )
				{
					Open.Emplace ( Index - MazeSize.X );
				}
				if ( Z - 1 >= 0 )
				{
					Open.Emplace ( Index - JumpZ );
				}
				if ( X + 1 < MazeSize.X )
				{
					Open.Emplace ( Index + 1 );
...
bool ULFPMazeLibrary::GenerateMazeData ( UPARAM ( Ref )FLFPMazeTable& MazeTable , const TSet <int32>& PreConnection , const FIntPoint RoomExit , const FRandomStream& Seed )
{
		...
		for ( int32 i = 0 ; i < MazeTable.StartData.Num ( ) ; i++ )
		{
			switch ( MazeTable.StartData[ i ].CellType )
			{
				...
				case EMazeCellType::Maze_Room : RoomIndexs.Add ( i );
					break;
				case EMazeCellType::Maze_Close : break;
			}
		}
	...
	int32 CurrentIndex = NextVisitList.Num ( ) > 0
		                     ? *NextVisitList.begin ( )
		                     : -1;
	...
	while ( UnVisit.Num ( ) != 0 )
	{
		...
		// Next Index
		CurrentIndex = NextVisitList.Num ( ) > 0
			               ? *NextVisitList.begin ( )
			               : -1;
	}
	...
	for ( const int32 Item : RoomIndexs )
	{
		...
		for ( const int32 ConnectIndex : OpenIndex )
		{
			...
			{
				case EMazeCellType::Maze_Open : if ( CanConnect )
					{
					...
					break;
				case EMazeCellType::Maze_Room : MazeTable.MazeData[ Item ].OpenList.Add ( ConnectIndex );
					MazeTable.MazeData[ ConnectIndex ].OpenList.Add ( Item );
					break;
				default : ;
			}
...
{
	if ( MazeTable.MazeData.Num ( ) == 0 || Amount <= 0 || MazeTable.DeadEnd.Num ( ) < Amount )
	{
		return false;
	}
...
{
	if ( MazeTable.StartData.Num ( ) == 0 || AreaSize.X <= 0 || AreaSize.Y <= 0 )
	{
		return false;
	}
...
TArray <int32> ULFPMazeLibrary::GetMazeCellNeighbourByType ( const FLFPMazeTable& MazeTable , const int32 Index , const EMazeCellType Type )
{
	...
	if ( !MazeTable.StartData.IsValidIndex ( Index ) )
	{
		return ReturnData;
	}
