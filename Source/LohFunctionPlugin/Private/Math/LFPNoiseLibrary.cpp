...
FVector ULFPNoiseLibrary::GetVectorNoise ( const FLFPNoiseTable& NoiseTable , const FIntVector Location , const bool bRoundNoise )
{
	...
	float a = NoiseTable.NoiseData[ ( Location.X < 0
		                                  ? 256 - FMath::Abs ( Location.X )
		                                  : Location.X ) % 256 ];
	float b = NoiseTable.NoiseData[ ( ( Location.Y < 0
		                                    ? 256 - FMath::Abs ( Location.Y )
		                                    : Location.Y ) + 64 ) % 256 ];
	float c = NoiseTable.NoiseData[ ( ( Location.Z < 0
		                                    ? 256 - FMath::Abs ( Location.Z )
		                                    : Location.Z ) + 128 ) % 256 ];
	...
	return bRoundNoise
		       ? FVector ( FMath::RoundToFloat ( Result.X ) , FMath::RoundToFloat ( Result.Y ) , FMath::RoundToFloat ( Result.Z ) )
		       : Result;
}
...
FVector ULFPNoiseLibrary::GetDirectionNoise ( const FLFPNoiseTable& NoiseTable , const FIntVector Location , const bool bRoundNoise )
{
	...
	return
			UKismetMathLibrary::GetForwardVector ( bRoundNoise
				                                       ? FRotator (
		...
		for ( int32 Y = -1 ; Y <= 1 ; Y++ )
		{
			for ( int32 X = -1 ; X <= 1 ; X++ )
			{
				...
				FLFPNearbyVectorData CurrentData (
				                                  IgnoreZ
					                                  ? FVector::Dist2D ( Location , FVector ( CurrentLocation ) + CurrentNoiseDirection )
