...
void ULFPTickManager::TickComponent ( float DeltaTime , ELevelTick TickType , FActorComponentTickFunction* ThisTickFunction )
{
	...
	if ( TickActorList.Num ( ) == 0 || CurrentCallCount != 0 )
	{
		return;
	}
...
{
	if ( !IsActive ( ) || TickActorList.Num ( ) == 0 || !GetOwner ( ) )
	{
		return -1;
	}
...
void ULFPTickManager::SendTickInternal ( )
{
	for ( int32 Index = 0 ; Index < TickPerCall ; Index++ )
	{
		...
		while ( TickActorList.Num ( ) != 0 )
		{
			...
			CurrentArrayIndex = TickActorList.Num ( ) <= CurrentArrayIndex
				                    ? 0
				                    : CurrentArrayIndex;
		...
		if ( --CurrentCallCount == 0 || TickActorList.Num ( ) == 0 )
		{
			break;
		}
	}
	...
	{
		GetWorld ( )->GetTimerManager ( ).SetTimerForNextTick ( this , &ULFPTickManager::SendTickInternal );
	}
...
bool ULFPTickManager::AddTickActor ( AActor* Actor , const bool IsRandom , const bool CheckDuplication )
{
	...
	if ( CheckDuplication && TickActorList.Contains ( Actor ) )
	{
		return true;
	}

	TickActorList.EmplaceAt ( IsRandom
		                          ? Seed.RandHelper ( TickActorList.Num ( ) )
		                          : TickActorList.Num ( ) , Actor );
