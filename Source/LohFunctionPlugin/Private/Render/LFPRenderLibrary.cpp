...
UTexture2D* ULFPRenderLibrary::CreateTexture2D( const FIntPoint Size , const TextureFilter Filter , const bool bSRGB )
{
	...
	VoxelColorMap->MipGenSettings = TMGS_NoMipmaps;
#endif
	VoxelColorMap->CompressionSettings = TC_VectorDisplacementmap;
...
{
	return {
			(Scale * Rotation.RotateVector(FVector(-1.0f, -1.0f, 1.0f))) + Center
			, (Scale * Rotation.RotateVector(FVector(1.0f, -1.0f, 1.0f))) + Center
			, (Scale * Rotation.RotateVector(FVector(-1.0f, 1.0f, 1.0f))) + Center
			, (Scale * Rotation.RotateVector(FVector(1.0f, 1.0f, 1.0f))) + Center
		};
